...
document.addEventListener("DOMContentLoaded", function () {
  ...
  //const oldNode = btnPagar.cloneNode(true);
  actualizarTotalCarrito();
  //btnPagar.parentNode.replaceChild(oldNode, btnPagar);
    ...
    carrito.forEach((item, index) => {
      if (carrito.length - 1 != index) {
        ...
        if ("papas" in item) {
          ...
          contenidoItem = `
                    <p class="itemNombre">${item.nombre
            }<span class="itemprecio">${item.precio.toFixed(
              2
            )}</span></p>
                    <p class="itembebida">${item.bebida}</p>
                    <p class="itemcantidad">Cantidad: <span class="itemcannum">${item.cantidad
...
function construirObjetoCarrito(elementosCarrito) {
  ...
  elementosCarrito.forEach((item) => {
    if (item.querySelector(".itempapas")) {
      ...
      //console.log('con papas' + itemCarrito.papas);
    } else {
      const nombreElement = item.querySelector(".itemNombre");
      const nombre = nombreElement.childNodes[0].textContent;
      const precio = parseFloat(item.querySelector(".itemprecio").textContent);
      ...
      //console.log('sin papas' + itemCarrito);
    }
  });
...
productos.forEach((producto) => {
  producto.addEventListener("click", (e) => {
    ...
    const modalCorrespondiente = document.querySelector(
      `.modal-item[data-item="${itemNumber}"]`
    );
    ...
    modalCorrespondiente.querySelector(".modal__paragraph").textContent =
      descripcion;
    // Aquí, ajusta según tu estructura específica del precio
    modalCorrespondiente.querySelector("#total").textContent =
...
function agregarEventos(modal, producto) {
  ...
  agregarAlCarritoBtn.addEventListener("click", function () {
    ...
    const papas = modal.querySelector('input[name="papas"]:checked')
      .parentElement.innerText;
    const bebida = modal.querySelector('input[name="bebida"]:checked')
      .parentElement.innerText;
    //const cantidad = parseInt(modal.querySelector('#cantidad').textContent);
    //const precioTotalItem = precio * cantidad;
    ...
    if (sinPapas) {
      nuevoItemCarrito.innerHTML = `
            <p class="itemNombre">${nombre} $<span class="itemprecio">${precio.toFixed(
        2
      )}</span></p>
            <p class="itembebida">${bebida}</p>
            <p class="itemcantidad">Cantidad: <span class="itemcannum">${cantidad}</span></p>
...
function actualizarTotalCarrito() {
  ...
  if (itemsCarrito.length === 0) {
    carritoLista.innerHTML =
      '<div id="cuadro" style="padding: 0px; text-align: center;"><p id="mensajevacio" >Carrito sin productos</p></div>';
  } else {
